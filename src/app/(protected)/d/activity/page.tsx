...
import { Activity, ShieldCheck, RefreshCw, Undo2, Check, X } from "lucide-react";
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-text-3 text-[13px] mt-1">
            Every action Alfred takes is logged here — green runs free, amber is reversible, red waits for you.
          </p>
        </div>
        <button
          onClick={load}
          className="flex items-center gap-1.5 text-[12px] text-text-2 hover:text-text-1 transition-colors"
        >
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="text-text-3 text-[13px] mt-1">
                The safety layer is in place: actions are tier-classified, the ledger is recording, and money
                movement is impossible by design. Once Alfred starts acting, every move shows up here.
              </div>
            </div>
          </div>
        </Card>
            ...
            {actions.map(a => (
              <li key={a.id} className="flex items-center gap-3 px-4 py-3">
                <span
                  ...
                  style={{
                    background:
                      a.tier === "green" ? "#34d399" : a.tier === "amber" ? "#fbbf24" : "#f87171",
                  }}
                />
                ...
                {a.status === "proposed" ? (
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      ...
                      className="flex items-center gap-1 text-[11px] px-2 py-1 rounded-md border border-[rgba(52,211,153,0.3)] text-success hover:bg-[rgba(52,211,153,0.1)] transition-colors disabled:opacity-50"
                      title="Approve — this will send"
                    >
                    ...
                    <button
                      ...
                      disabled={undoing === a.id}
                      className="flex items-center gap-1 text-[11px] px-2 py-1 rounded-md border border-border-dim text-text-2 hover:text-danger transition-colors disabled:opacity-50"
                      title="Deny"
                    >
                      <X size={12} /> Deny
                    </button>
                  ...
                  <button
                    onClick={() => undo(a.id)}
                    disabled={undoing === a.id}
                    className="flex items-center gap-1 text-[11px] text-text-2 hover:text-danger transition-colors shrink-0 disabled:opacity-50"
                    title="Undo this action"
                  >
                    <Undo2 size={12} /> {undoing === a.id ? "…" : "Undo"}
                  </button>
                ) : null}
